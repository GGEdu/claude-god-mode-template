...
| `continuous-learning` | Instinct-based learning with confidence scoring, evolves into skills/commands/agents |
...
Note: `continuous-learning` and `continuous-learning` have extra files (config.json, hooks, scripts) — ensure the entire directory is copied, not just SKILL.md.
