---
name: ui-engineer
description: "UI/UX specialist for building new interfaces and improving existing ones. BUILD mode commits to an aesthetic direction first (impeccable's direction roll, never the category default) and records it so every later screen reuses it; REFACTOR mode applies the direction or impeccable's refine commands (bolder, quieter, distill, polish…). Quality floor: accessibility, touch, performance. Verifies with impeccable's detector and one batched screenshot round."
